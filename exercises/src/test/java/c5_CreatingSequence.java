...
public class c5_CreatingSequence {
    ...
    public void interval() {
        Flux<Long> interval = Flux.interval(Duration.ofSeconds(3L)).delayElements(Duration.ofMillis(900)); //todo: change this line only
    ...
    public void repeat() {
        ...
        Callable<Integer> a = counter::getAndIncrement;
        Flux<Integer> repeated = Mono.fromCallable(a).repeat(9L).log();//todo: change this line
    ...
    public void generate_programmatically() {
        ...
        Flux<Integer> createFlux = Flux.create(sink -> {
            //todo: fix following code so it emits values from 0 to 5 and then completes
            IntStream.range(0,6)
                    .peek(i-> System.out.println("create: "+1+" in: "+Thread.currentThread().getName()))
                    .forEach(sink::next);
        ...
        Flux<Integer> pushFlux = Flux.push(sink -> {
            //todo: fix following code so it emits values from 0 to 5 and then completes
            System.out.println("push: "+a.get());
            IntStream.range(0,6)
                    .peek(i-> System.out.println("push: "+1))
                    .forEach(sink::next);
            sink.complete();
        });
